...
const char* deterStructtype(int year, int bldtypeid, int story)
{
    ...
    if(year<=1900){
        ...
        if(story<4){
            if(bldtypeid==1||bldtypeid==2||bldtypeid==3||bldtypeid==12){  //1-12 Residential 13-19 Commercial 20-22 Industrial 23-25 Other
                structingtypeID=1;
            }
            ...
            if(bldtypeid<7&&bldtypeid>0||bldtypeid>9&&bldtypeid<15){  //1-12 Residential 13-19 Commercial 20-22 Industrial 23-25 Other
...
int main(int argc, const char **argv) {
  ...
  while ((row = CsvParser_getRow(csvparser))) {
	...
	//	return 0;
      ...
      const char *type = deterStructtype(atoi(rowFields[11]),atoi(rowFields[15]),atoi(rowFields[10]));
      //      printf("%s\n",type);
  
      json_object_set(GI,"structType",json_string(type));// deterStructtype(atoi(rowFields[11]),atoi(rowFields[15]),atoi(rowFields[10]))));
